...
def client_job(embedding_size, global_weight, client_idx, num_clients, batch_size, initial_lr, num_client_epoch, local_weight, loss_locals, embedding_dim=100):
    local = Client(embedding_size=embedding_size, weight=copy.deepcopy(global_weight), client_idx=client_idx,
                   num_clients=num_clients, batch_size=batch_size, initial_lr=initial_lr, num_epoch=num_client_epoch)
    ...
    for epoch in range(num_epoch):
        ...
        for idx in idxs_users:
            t = threading.Thread(target=client_job, args=(embedding_size, global_weight, idx, num_clients, batch_size, initial_lr,
                                                          num_client_epoch, local_weight, loss_locals, embedding_dim))
            t.start()
            threads.append(t)
