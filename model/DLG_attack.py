...
def DLG_infer(model_index=0, num_clients=10, batch_size=10, embedding_dim=100, use_GPU=True, num_epoch=10):
    ...
    # 制作测试集
    data = InputData(file_name='logs/random_walk_trace_test.txt', min_count=10)
    ...
    for i in range(int(batch_count)):
        ...
        pos_u = torch.autograd.Variable(torch.LongTensor(pos_u), requires_grad=False)
        pos_v = torch.autograd.Variable(torch.LongTensor(pos_v), requires_grad=False)
        neg_v = torch.autograd.Variable(torch.LongTensor(neg_v), requires_grad=False)
